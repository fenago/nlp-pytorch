...
*[]{#_idTextAnchor070}Chapter 4*: Text Preprocessing, Stemming, and Lemmatization {#_idParaDest-70}
=================================================================================
...
Text preprocessing {#_idParaDest-72}
==================

::: {#_idContainer141}
Textual data can come in a variety of formats and styles. Text may be in
a structured, readable format []{#_idIndexMarker201}or in a more raw,
...
detail.[]{#_idTextAnchor073}

[]{#_idTextAnchor074}

Removing HTML {#_idParaDest-73}
-------------

When scraping text from online sources, you may find that your text
contains HTML markup and []{#_idIndexMarker202}other non-textual
...
HTML, the `<b>`{.literal} tag indicates that the text following it
...
`BeautifulSoup`{.literal} that allows us to remove all HTML in a few
lines:
...
![Figure 4.1 -- Removing HTML ](3_files/B12365_04_01.jpg)
:::
...
data[]{#_idTextAnchor075}.

[]{#_idTextAnchor076}

Converting text into lowercase {#_idParaDest-74}
------------------------------

It is standard practice when preprocessing text to convert everything
into lowercase. This is because []{#_idIndexMarker203}any two words that
are the same should be considered semantically identical, regardless of
whether they are capitalized or not. \'`Cat`{.literal}\',
\'`cat`{.literal}\', and \'`CAT`{.literal}\' are all the same words but
...
It is worth []{#_idIndexMarker204}noting that capitalization may be
...
(the country) and *turkey* (the bird[]{#_idTextAnchor077}).

[]{#_idTextAnchor078}

Removing punctuation {#_idParaDest-75}
--------------------

Sometimes, depending on the type of model being constructed, we may wish
to remove punctuation []{#_idIndexMarker205}from our input text. This is
...
this in Python by using the `re`{.literal} library, to match any
punctuation using a regular expression, and the `sub()`{.literal}
...
punctuation. A good example would []{#_idIndexMarker206}be the use of
the ampersand (`&`{.literal}), which in almost every instance is used
interchangeably with the word \"`and`{.literal}\". Therefore, rather
than completely removing the ampersand, we may instead opt to replace it
directly with the word \"`and`{.literal}\". We can easily implement this
in Python using the` .replace()`{.literal} function:
...
may be essential for []{#_idIndexMarker207}the representation of a
sentence. One crucial example is email addresses. Removing the
`@`{.literal} from email addresses doesn\'t make the address any more
readable:

`name@gmail.com`{.literal}
...
mod[]{#_idTextAnchor079}el.

[]{#_idTextAnchor080}

Replacing numbers {#_idParaDest-76}
-----------------
...
eight, seven). It may be worth transforming []{#_idIndexMarker208}these
...
![Figure 4.5 -- Replacing numbers with text ](3_files/B12365_04_05.jpg)
:::
...
However, in a []{#_idIndexMarker209}similar fashion to processing email
...
Clearly, the input []{#_idIndexMarker210}in the preceding example is a
...
numbers from our input t[]{#_idTextAnchor081}ext.


Stemming and lemmatization {#_idParaDest-77}
==========================
...
word. This often involves changing the []{#_idIndexMarker211}prefix or
suffix of a word but can also involve modifying
[]{#_idIndexMarker212}the entire word. For example, we can make
...
sec[]{#_idTextAnchor083}tions.

[]{#_idTextAnchor084}

Stemming {#_idParaDest-78}
--------

**Stemming** is []{#_idIndexMarker213}the algorithmic process by which
...
follow a []{#_idIndexMarker214}particular algorithm in order to return
the stem of a word. In English, one of the most common stemmers is the
Porter Stemmer.

The **Porter Stemmer** is an []{#_idIndexMarker215}algorithm with a
...
2.  We then simply call this instance of the stemmer on individual words
    ...
    ](4_files/B12365_04_07.jpg)
    :::

    Figure 4.7 -- Returning the stems of words

3.  We can also []{#_idIndexMarker216}apply stemming to an entire
...
Stemmer. Some words, such as `stemming`{.literal} and
`timing`{.literal}, reduce to their expected stems of `stem`{.literal}
and `time`{.literal}. However, some words, such as `saw`{.literal},
don\'t reduce to their logical stem (`see`{.literal}). This illustrates
...
We can examine []{#_idIndexMarker217}some of the rules the Porter
...
the use of lemmati[]{#_idTextAnchor085}zation.

[]{#_idTextAnchor086}

Lemmatization {#_idParaDest-79}
-------------
...
[]{#_idIndexMarker218}true lexical root. So, while the stem of the word
`ran`{.literal} will just be *ran*, its lemma is the true lexical root
of the word, which would be `run`{.literal}.
...
[]{#_idIndexMarker219}example, we will look at using the **WordNet**
...
all the words in the English language, it knows that `mice`{.literal} is
the plural version of `mouse`{.literal}. We would not have been able to
...
[]{#_idIndexMarker220}sources where language doesn\'t necessarily match
...
This means []{#_idIndexMarker221}that in order to return the correct
...
::: {#_idContainer138 .IMG---Figure}
![Figure 4.14 -- Output of POS tagging on a sentence
](4_files/B12365_04_14.jpg)
:::
...
POS. This means that we first create []{#_idIndexMarker222}a function
...
notable exceptions. When we might decide to []{#_idIndexMarker223}use
stemming and lemmatization depends on the requirements of the task at
hand, some of which we will explo[]{#_idTextAnchor087}re now.


Uses of stemming and lemmatization {#_idParaDest-80}
==================================

::: {#_idContainer141}
Stemming []{#_idIndexMarker224}and lemmatization are both a form of NLP
that can be used to extract information from text. This
[]{#_idIndexMarker225}is known as **text mining**. Text mining tasks
come []{#_idIndexMarker226}in a variety of categories, including text
...
[]{#_idIndexMarker227}and by reducing words to their roots or lemmas, we
can efficiently analyze sentences within our deep learning frameworks.
If we are []{#_idIndexMarker228}able to reduce a 10-word sentence to
...
Differences in lemmatization and stemming {#_idParaDest-81}
-----------------------------------------

Now that we have seen both lemmatization and stemming in action, the
question still remains as to []{#_idIndexMarker229}under which
...
[]{#_idIndexMarker230}may just be a reduced form of the target room,
